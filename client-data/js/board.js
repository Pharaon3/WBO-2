...
(function () {
    ...
    function createPNG(svg) {
	    ...
	    const box = Tools.svg.querySelector("#drawingArea").getBBox();
...
Tools.generateUID = function (prefix, suffix) {
	...
	uid += (Math.round(Math.random()*1e19)).toString(36); //Add a random character at the end
...
Tools.setColor = function (color) {
    ...
    if (Tools.targets && Tools.list.Selector) {
...
function colorUpdate(data){
	for (let elem of data) {
	  ...
	  Tools.drawAndSend(msg, Tools.list.Selector);
	}
}
